-- Migration: enhance_memory_personality_system
-- Created at: 1761630608
...
ADD COLUMN IF NOT EXISTS source_conversation_id UUID REFERENCES transcripts(id),
...
-- 5. Insert default privacy preferences
...
-- 6. Create memory importance calculation triggers
CREATE OR REPLACE FUNCTION update_memory_importance()
RETURNS TRIGGER AS $$
BEGIN
  NEW.importance_score = GREATEST(
    NEW.importance_score,
    CASE 
      WHEN NEW.emotional_weight > 0.7 THEN NEW.emotional_weight * 10
      WHEN NEW.emotional_weight > 0.4 THEN NEW.emotional_weight * 7
      ELSE NEW.emotional_weight * 5
    END
  );
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS memory_importance_trigger ON memory;
...
-- 8. Update personality_state to include adaptive learning features
