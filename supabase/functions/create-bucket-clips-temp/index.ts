...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["video/mp4", "video/webm", "audio/mpeg", "image/jpeg"];
    bucketConfig.file_size_limit = 104857600;
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for clips" ON storage.objects FOR SELECT USING (bucket_id = 'clips');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for clips" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'clips');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for clips" ON storage.objects FOR UPDATE USING (bucket_id = 'clips');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for clips" ON storage.objects FOR DELETE USING (bucket_id = 'clips');`
    ];
