...
CREATE TABLE IF NOT EXISTS public.clips (
    ...
    highlight_reason TEXT, -- 'sentiment_peak', 'funny_moment', 'surprise', 'user_engagement', etc.
    sentiment_score NUMERIC, -- Sentiment value at this moment
    
    -- File information
    formats_available JSONB DEFAULT '[]'::jsonb, -- ['mp4', 'webm', 'mp3']
    thumbnail_url TEXT,
    clip_urls JSONB DEFAULT '{}'::jsonb, -- {mp4: 'url', webm: 'url', mp3: 'url'}
    ...
    CONSTRAINT valid_duration CHECK (duration <= 300) -- Max 5 minutes
);
...
-- Create clip_shares table for tracking share analytics
...
-- Create user_storage_quota table for tracking storage usage
CREATE TABLE IF NOT EXISTS public.user_storage_quota (
    user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    total_storage_bytes BIGINT DEFAULT 0,
    clip_count INTEGER DEFAULT 0,
    quota_limit_bytes BIGINT DEFAULT 524288000, -- 500MB default
...
-- Trigger to update storage quota when clip is completed
...
-- Function to decrement storage quota on clip deletion
...
-- Trigger to decrement storage quota when clip is deleted
...
BEGIN
    -- Insert share record
    INSERT INTO public.clip_shares (clip_id, platform)
    VALUES (clip_id_param, platform_param);
...
-- Function to clean up old clips (30 days)
...
-- Public clips can be viewed by anyone (for future sharing feature)
-- CREATE POLICY "Public clips viewable by all"
--     ON public.clips FOR SELECT
--     USING (is_public = true);

-- Anyone can view clip shares (for analytics)
CREATE POLICY "Clip shares viewable by clip owner"
    ON public.clip_shares FOR SELECT
    USING (EXISTS (
        SELECT 1 FROM public.clips
        WHERE clips.id = clip_shares.clip_id
        AND clips.user_id = auth.uid()
    ));

-- Anyone can insert clip shares (for tracking)
CREATE POLICY "Anyone can record clip shares"
    ON public.clip_shares FOR INSERT
    WITH CHECK (true);

-- Users can view their own storage quota
CREATE POLICY "Users can view own storage quota"
    ON public.user_storage_quota FOR SELECT
    USING (auth.uid() = user_id);

-- System can update storage quota
CREATE POLICY "System can update storage quota"
    ON public.user_storage_quota FOR UPDATE
    USING (true);

CREATE POLICY "System can insert storage quota"
    ON public.user_storage_quota FOR INSERT
    WITH CHECK (true);

-- Function to check if user has storage quota
...
BEGIN
    SELECT 
        COALESCE(total_storage_bytes, 0),
        COALESCE(quota_limit_bytes, 524288000)
...
COMMENT ON TABLE public.clips IS 'Stores generated audio/video clips from conversations';
COMMENT ON TABLE public.clip_shares IS 'Tracks clip sharing analytics across platforms';
COMMENT ON TABLE public.user_storage_quota IS 'Tracks user storage usage and quota limits';
